    ...
    public class ContestTypeServiceTests
    {
        ...
        public async Task GetAll_ShouldReturnCollection()
        {
            ...
            using (var context = new FullFraimDbContext(options))
            {
                ...
                //Assert
                Assert.AreEqual(2, result.Count());
                Assert.AreEqual(Constants.ContestTypeSeed.Invitational, result[0].Name);
                Assert.AreEqual(Constants.ContestTypeSeed.Open, result[1].Name);
        ...
        public async Task GetById_ShouldReturnCorrect_WhenIdIsValid()
        {
            ...
            using (var context = new FullFraimDbContext(options))
            {
                ...
                Assert.AreEqual(Constants.ContestTypeSeed.Invitational, result.Name);
                Assert.AreEqual(Constants.ContestTypeSeed.Open, result2.Name);
